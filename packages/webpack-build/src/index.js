...
const commongPlugins = require('./config/webpack.commons.plugins');
const prodConfig = require('./config/webpack.production');
const devConfig = require('./config/webpack.development');
const bundleanalyzer = require('./config/addons/webpack.bundleanalyzer');
const bundleBuddy = require('./config/addons/webpack.bundlebuddy');
const fileReader = require('./config/readFiles');
const { envObjectForWebpack } = require('./config/webpack.env');
...
const configureWebpack = (env) => {
  env = { ...env, ...envObjectForWebpack }; //eslint-disable-line
  if (!env.app) {
  ...
  if (env.app !== '*') {
    ...
    if (!app) {
      ...
      console.log(env);
      return build(env); //eslint-disable-line
    }
  }
  /**
  ...
  return Object.keys(appManifest.applications).map((app) => {
    ...
    console.log(copyEnv);
    return build(copyEnv); //eslint-disable-line
  });
};
