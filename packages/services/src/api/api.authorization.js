...
} from '@healthifyme/constants/dist/app/app.constants';
import { APP_LOGIN } from '@healthifyme/constants/dist/api/api.endpoints';
...
export const getNewApiUrl = (url) => {
  ...
  let auth;
  let newUrl = url;
  if (process.env.PROXY_ORIGIN === 'staging') {
    auth = STAGE_ACCESS;
    if (url.indexOf('?') === -1) {
      newUrl = `${newUrl}?${auth}`;
    } else {
