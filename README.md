...
this project uses `lerna` and `yarn workspaces` to implement `mono repo` and `local dependency` management to share code across all the applications
...
4. install lerna `npm install --global lerna`
...
```
.
├── /__mocks__/                # contains the actual source code
